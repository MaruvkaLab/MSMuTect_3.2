...
def validate_indexing(bam_files: List[str]) -> None:
    ...
    ###FI switch to using the reads fetcher??
    prefixes = ['', 'chr', 'Chr']
...
def validate_output_files(arguments: argparse.Namespace) -> None:
    overwrite_files_msg = "Files would be overwritten by this run. To force overwrite, use -f flag"
    if arguments.force:
        return
    elif arguments.single_file:
        if arguments.histogram and not arguments.allele:
            if os.path.exists(arguments.output_prefix + ".hist.tsv"):
                exit_on(overwrite_files_msg)
        elif not arguments.msi_detect: # alleles should be calculated
...
def validate_msi_detect(arguments: argparse.Namespace) -> None:
    if arguments.tumor_file or arguments.normal_file:
        exit_on("MSI Detect only works on single files. See -S flag")
    elif not arguments.noise_file:
        exit_on("MSI Detect requires noise file. See -n flag")
    elif arguments.histogram or arguments.allele or arguments.mutation:
        exit_on("MSI Detect cannot be used with -mutation, -allele, or  -histogram flags")
    elif arguments.loci_file:
        exit_on("MSI Detect uses the noise file, not the loci file")
    elif not(arguments.noise_file) or os.path.exists(arguments.noise_file):
        exit_on("Given noise file does not exist")


def validate_msmutect(arguments: argparse.Namespace) -> None:
    if not arguments.loci_file or not os.path.exists(arguments.loci_file):
...
def validate_input(arguments: argparse.Namespace):
    ...
    if arguments.msi_detect:
        validate_msi_detect(arguments)
    else:
        validate_msmutect(arguments)
