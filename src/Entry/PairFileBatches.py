...
def get_alleles(locus: Locus, reads_fetcher: ReadsFetcher, flanking: int, noise_table) -> AlleleSet:
    ...
    alleles = calculate_alleles(histogram, noise_table)
    return alleles


def partial_full_pair(loci: List[Locus], normal: str, tumor: str, flanking: int, noise_table) -> List[str]:
    if len(loci) == 0:
        return []
    calls = []
    ...
    fisher = Fisher()
    for locus in loci:
        normal_alleles = get_alleles(locus, normal_fetcher, flanking, noise_table)
