...
def should_process_read(read, locus_start, locus_end, flanking, removed_bases, p_exclude):
    ...
    for cigar_op in read.cigartuples:
        if cigar_op[0] in [CIGAR_OPTIONS.ALG_MATCH, CIGAR_OPTIONS.DELETION, CIGAR_OPTIONS.REF_SKIP,
                           CIGAR_OPTIONS.SEQ_MATCH, CIGAR_OPTIONS.SEQ_MISMATCH]:
    ...
    for locus in loci_list:
        ...
        for read in reads_iter:
            if should_process_read(read, start, end, flanking, removed_bases, p_exclude):
                res, read_fraction, read_deletion_overflow, read_non_msi_insertion = process_single_read(start, end,
                                                                                                         num_repeats,
                                                                                                         pattern, read)
                if res in res_dict:
                    res_dict[res] += 1
        ...
        final_str_for_read = "{0}:{1}:{2}:{3}:{4}, {5}\t{6}\t{7}\t{8}".format(chrom, start, end, pattern, num_repeats,
                                                                              res_string, fraction_flag,
                                                                              overflow_deletion_flag,
                                                                              non_msi_insertion_flag)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r", "--removed_bases", help="Number of bases to be removed at the end of the read.",
                        default=0)
    ...
    with Pool() as p:
        ...
        for line in loci_list:
            if lines_count == int(args.batch_size):
                ...
                results.append(p.apply_async(process_loci, args=(
                lines_list, args.input_BAM, int(args.flanking), int(args.removed_bases), float(args.exclude))))
                lines_count = 0
                lines_list = []
            else:
                lines_list.append(line)
