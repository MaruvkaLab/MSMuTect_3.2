...
class TestAnnotatedLocus(unittest.TestCase):
    ...
    def test_full_pipeline_runs_elaborate(self):
        ...
        ref_repeats = [3, 7, 4, 49][:1]
        locus_hists = [
            [[3, 6], [5, 5], [2, 1]],
            [[7, 12], []]
        ]
        full_match_repeat_support = [12, 12, 12, 12]
        # num_repeats = [2, 1, 1, 1]
        all_res = list(res_reader)
        for i in range(len(ref_repeats)):
            print(i)
            ...
            num_reps = len(current_locus_hist)
            print(res.normal_motif_repeats)
            print(res.normal_motif_repeat_support)
